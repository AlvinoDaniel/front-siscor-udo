          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                v-bind="attrs"
                v-on="on"
                @click="getBandejaBorradores(true)"
              >
           ...
           <template v-slot:no-data>
            <v-row>
              <v-col cols="12" class="d-flex flex-column justify-center align-center pa-12">
                <v-img
                  :src="require('@/assets/Icons/ICONO_BORRADOR.png')"
                  max-width="200"
                  style="opacity: .7;"
                />
                <span class="text-h5 font-weight-bold blue-grey--text">No tiene Borradores</span>
                <span class="text-h5 blue-grey--text">Empiece a redactar documentos y guardalo como borrador</span>
              </v-col>
            </v-row>
           </template>
        </v-data-table>
...
export default {
  ...
  methods: {
    async getBandejaBorradores (actualizar=false) {
