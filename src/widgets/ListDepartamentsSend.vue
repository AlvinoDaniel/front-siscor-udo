    ...
    <v-card>
      ...
      <v-card-text class="pa-0">
        <v-row class="ma-0">
                ...
                <v-list-item class="">
                  ...
                  <v-list-item-action>
                    <v-list-item-action-text>
                      {{ item.pivot.fecha_leido | smartDate }}
                    </v-list-item-action-text>
                  </v-list-item-action>
                </v-list-item>
                <v-divider
          ...
          <!-- <v-col cols="12">
            <v-tabs v-model="selected" dense color="icono">
              <v-tab class="text-subtitle text-capitalize">Leidos por</v-tab>
              <v-tab class="text-subtitle text-capitalize">Recibido por</v-tab>
            </v-tabs>
            <v-divider />
    ...
    computed: {
      leidos () {
        return this.items.length > 0
          ? this.items.filter(dpto => dpto?.pivot?.leido === 1)
          : []
      },
      noLeido () {
        return this.items.length > 0
          ? this.items.filter(dpto => dpto?.pivot?.leido === 0)
          : []
      },
      copiasItems () {
        return this.items.length > 0
